...
### 🚀 "딸깍" 실행 (Docker 전용)
별도의 복잡한 설치 없이 명령어 한 번으로 전체 시스템(DB, 백엔드, 프론트엔드)을 구동하고 테스트 데이터까지 완벽하게 세팅합니다.

```bash
# 전체 시스템 빌드 및 백그라운드 실행
docker-compose up --build -d
```

*   **웹 접속**: [http://localhost:3000](http://localhost:3000)
*   **API 문서**: [http://localhost:8000/docs](http://localhost:8000/docs)
*   **외부 DB 접속**: `localhost:5433` (User: degururu / Pass: degururu2026)

---

## 🧪 테스트 계정
구동 즉시 아래 계정으로 모든 기능을 테스트해 보실 수 있습니다.

| 역할 | 이메일 (ID) | 비밀번호 |
|------|--------|----------|
| **관리자** | `admin@degururu.com` | `admin1234` |
| **일반 회원** | `member1@gmail.com` | `member1234` |
